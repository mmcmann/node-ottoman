...
class Pouf {
  ...
  // Define the special `bucket` property which allows you to quickly specify a
  // Couchbase Bucket to use without messing with the underlying StoreAdapters.
  get bucket() {
   ...
   */
  public model(name, schemaDef, options) {
   ...
   */
  public validate(mdlInst, callback) {
   ...
   */
  public plugin(pluginFn, options) {
   ...
   * @returns {boolean}
   */
  public isModel(model) {
    return model.super_ === Model;
  }
   ...
   * @ignore
   */
  private _parseFieldType(options) {
    if (!options) {
   ...
   * @ignore
   */
  private _makeField(name, fieldDef) {
    if (name === "id") {
   ...
   * @ignore
   */
  private _createSchema(name, schemaDef, options) {
   ...
   * @ignore
   */
  private _findModelsByRefDocIndex(model, fields, values, options, callback) {
   ...
   * @ignore
   */
  private _findModelByIndex(model, index) {
   ...
   * @ignore
   */
  private _countModels(model, filter, options, callback) {
   ...
   * @ignore
   */
  private _normFilter(filter) {
    var out = {};
    for (var i in filter) {
      if (filter.hasOwnProperty(i)) {
        if (filter[i] instanceof Model) {
          out[i] = {
            $ref: filter[i].id()
          };
   ...
   * @ignore
   */
  private _findModels(model, filter, options, callback) {
    if (options instanceof Function) {
      callback = options;
      options = {};
    }
   ...
   * @ignore
   */
  private _findModelsByQuery(mdlInst, model, query, callback) {
   ...
   * @ignore
   */
  private _buildModel(name, schemaDef, options) {
    ...
    model.namePath = Model.namePath;
    model.ref = Model.ref;
    model.refByKey = Model.refByKey;
    model.plugin = Model.plugin;
    model.getById = Model.getById;
    model.fromData = Model.fromData;
    model.pre = Model.pre;
    model.post = Model.post;
    model.loadAll = Model.loadAll;
   ...
   * @ignore
   */
  private _delayBind(bindType, fn) {
   ...
   * @ignore
   */
  private _buildAndRegisterModel(name, schemaDef, options) {
   ...
   * @ignore
   */
  private _buildAndRegisterTypeDef(name, fieldDef) {
    if (this.typeByName(name)) {
      throw new Error("A model with this name has already been registered.");
    }
  ...
  private _ensureModelIndices(model, callback) {
    var schema = model.schema;
  ...
  static loadAll = Model.loadAll;
  static StoreAdapter = StoreAdapter;
  static StoreAdapter.Couchbase = CbStoreAdapter;
  static StoreAdapter.Mock = MockStoreAdapter;
  static CbStoreAdapter = Pouf.StoreAdapter.Couchbase;
  static MockStoreAdapter = Pouf.StoreAdapter.Mock;
  static Consistency = StoreAdapter.SearchConsistency;
}

export default new Pouf();
